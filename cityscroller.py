...
class Scroller(object):
    ...
    def add_buildings(self):
        ...
        x = 0
        while x <= self.width:
            w = self.add_building(x)
            x += w
            # add width of that building to x
    def add_building(self, x_location):
        ...
        """
        tallest = self.base - self.height
        shortest = tallest//4
        building_height = random.randint (shortest, tallest)
        widest = self.width//4
        thinnest = widest//4
        building_width = random.randint (thinnest, widest)
        y = self.base - building_height
        #color = random_color()
        myBuilding = Building(x_location, y, building_width, building_height, self.color)
        self.building_list.append(myBuilding)
    ...
    def move_buildings(self):
        ...
        x_spot = self.building_list[-1].x_point
        last_width = self.building_list[-1].width
        x_spot -= last_width
        self.add_building(x_spot)
